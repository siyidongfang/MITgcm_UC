...
%     SEAICE_no_slip          = false;
  ...
  % 
    ...
    trelaxmask = zeros(Nx,Ny,Nr);
    trelaxmask(:,:,1) = 1;

    writeDataset(trelaxvals,fullfile(inputpath,trelaxvalsFile),ieee,prec); 
    writeDataset(trelaxmask,fullfile(inputpath,trelaxmaskFile),ieee,prec); 

    rbcs_parm01.addParm('useRBCtemp',useRBCtemp,PARM_BOOL)
    rbcs_parm01.addParm('tauRelaxT',tauRelaxT,PARM_REAL)
    rbcs_parm01.addParm('relaxMaskFile',trelaxmaskFile,PARM_STR)
    rbcs_parm01.addParm('relaxTFile',trelaxmaskFile,PARM_STR)
