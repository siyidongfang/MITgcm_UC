...
%     useRBCS = false; %%% if melt = 0
  ...
  if(useErestWorlanski)      %%% Restoring to the east, and Orlanski to the west
      useobcsEast = true;
      useobcsWest = true;
      useRESTOReast = true;
      useRESTORwest = false;
  ...
  if(useErestWrest)           %%% Restoring to the east and west
      useobcsEast = true;
      useobcsWest = true;
      useRESTOReast = true;
      useRESTORwest = true;
...
%   N4 = 14;  
  ...
  h_interp = [0 -Htr 0 0];
%   % Add a trough over the continental shelf which does not extend to the
    ...
    %   Xmax = Xs;
     ...
     if(useSURFACE_SALT)
      ...
      for n=1:nForcingPeriods   
        ...
        salt_flux(polynya_idx_x,polynya_idx_y,n) = -constSaltFlux;  %%% <0, add salt to the ocean, increase SSS
...
%         end  
  ...
  else %%% No sea ice, relax surface temperature to freezing temperature across the entire domain
      %%%%% RELAXATION PARAMETERS %%%%%
      useRBCtemp = true;
      useRBCsalt = false;
      tauRelaxT = 6*t1hour;
