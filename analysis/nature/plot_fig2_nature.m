...
clf
load([prodir 'heatbudget_50km-100km.mat'])
Fheat_adv = -Fheat_adv;
...
rectangle('Position',[50 -0.2 50 2],'EdgeColor',[hex2rgb(red3) 0],'FaceColor',[boxcolor 0.2])
hold on;
plot(yy/1000,zeros(1,length(yy)),':','LineWidth',1,'color','k')
l2 = plot(yy/1000,Fheat_adv(2,:),'LineWidth',1.5,'color',blue2);
l3 = plot(yy/1000,Fheat_adv(3,:),'LineWidth',1.5,'color',blue3);
l4 = plot(yy/1000,Fheat_adv(4,:),'LineWidth',1.5,'color',blue4);
...
set(handle1,'Position',[0.40    0.225    0.005    0.63],'TickLength',0,'AxisLocation','in','Box','on','Color',black,'LineWidth',0.5);
str1 = flip({'33' '' '33.28' '' '33.56' '' '34.12' '' '34.38' '' '34.65' '' '34.78' '' '34.91'});
str2 = '{\it S}_{shelf}^{bot} (psu)';

box on
anno1 = annotation('textbox',[0.405 0 0.05 0.84],'String',str1,'FontSize',fontsize-1,'EdgeColor','none');     
...
refF = -F_band(5);
rectangle('Position',[33 refF 34.3825-33 3],'EdgeColor',[hex2rgb(red3) 0.3],'FaceColor',[hex2rgb(red3) 0.3])
rectangle('Position',[34.3825 -0.5 35-34.3825 0.5+refF],'EdgeColor',[hex2rgb(red3) 0.3],'FaceColor',[hex2rgb(red3) 0.3])

rectangle('Position',[33 -0.5 34.3825-33 0.5+refF],'EdgeColor',[bluefill 0.1],'FaceColor',[bluefill 0.1])
rectangle('Position',[34.3825 +refF 35-34.3825 3],'EdgeColor',[bluefill 0.1],'FaceColor',[bluefill 0.1])
...
%%% 2km, varying hi0
scatter(S(nexp+4),-F_band(nexp+4),sz*0.8,facecolor,'s','filled','MarkerEdgeColor',boxcolor); % 
lhi = scatter(S(nexp+5),-F_band(nexp+5),sz*1.3,facecolor,'s','filled','MarkerEdgeColor',boxcolor); % 
%%% 2km, varying Atide
scatter(S(nexp+6),-F_band(nexp+6),sz*0.8,facecolor,'p','filled','MarkerEdgeColor',boxcolor); % 
ltide = scatter(S(nexp+7),-F_band(nexp+7),sz*1.3,facecolor,'p','filled','MarkerEdgeColor',boxcolor); % 
%%% 2km, varying Ws
scatter(S(nexp+8),-F_band(nexp+8),sz*0.8,facecolor,'d','filled','MarkerEdgeColor',boxcolor); % 
lws = scatter(S(nexp+9),-F_band(nexp+9),sz*1.3,facecolor,'d','filled','MarkerEdgeColor',boxcolor); % 
...
scatter(S([9 21]+4),-F_band([9 21]+4),sz/2,hex2rgb(red3),'o','LineWidth',1)
scatter(S([9 21]+5),-F_band([9 21]+5),sz/2,hex2rgb(red1),'o','LineWidth',1)
...
box on;
title('{\it F}_{shelf}','FontSize',fontsize+2,'fontweight', 'normal')

leg21 = legend([l2km l5km l10km],...
    '2 km','5 km','10 km',...
    'FontSize', fontsize-2,'box','off');
set(leg21,'Position',  [0.76 0.6357 0.3309 0.2])

ah=axes('position',get(ax2,'position'),'visible','off');
leg22 = legend(ah,[lua lva ltide lhi lws],...
   'Varied U_a','Varied V_a','Varied A_{tide}','Varied h_{i0}','Varied W_S',...
   'FontSize', fontsize-2,'box','off');
set(leg22,'Position',  [0.77 0.2357 0.3309 0.4])

% anno3 = annotation('textbox',[0.6 0.23 0.1 0.1],'String',...
%     {'\it Positive' '\it feedback'},'FontSize',fontsize-1,'EdgeColor','none','Color',darkgray);     
% anno4 = annotation('textbox',[0.77 0.48 0.1 0.1],'String',...
%     {'\it Negative' '\it feedback'},'FontSize',fontsize-1,'EdgeColor','none','Color',darkgray);     
