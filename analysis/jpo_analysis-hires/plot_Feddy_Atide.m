...
%         expname = EXPNAME{n}
%         loadexp;
%         sloperange = SLOPERANGE(n);
%         yend = ystart + sloperange;
%         ymin = round(ystart/delY(1));
%         ymax = round(yend/delY(1));
%         yidx = ymin:ymax;
%         minDepth = min(min(abs(bathy(:,yidx)))) 
%         %%% The minimum depth over the slope ~ 890 m
%         
%     end
...
annotation('textbox',[0.05 panelbottom 0.05 0.05],'String','(a)','interpreter','latex','FontSize',fontsize,'LineStyle','None');
    p1 = plot(normalized_transient_slope_batch(3,:),-zz,'color','k','LineWidth',lw);
    hold on;
    p2 = plot(normalized_transient_slope_batch(1,:),-zz,'color',blue,'LineWidth',lw);
    p3 = plot(normalized_transient_slope_batch(2,:),-zz,'color',green,'LineWidth',lw);
    p4 = plot(normalized_transient_slope_batch(4,:),-zz,'color',yellow,'LineWidth',lw);
    p5 = plot(normalized_transient_slope_batch(5,:),-zz,'color',orange,'LineWidth',lw);
    hold off;
    ylim([100 845]); 
    xlim([0 1])
    axis ij;
...
annotation('textbox',[0.375 panelbottom 0.05 0.05],'String','(b)','interpreter','latex','FontSize',fontsize,'LineStyle','None');
    ...
    p1 = plot(normalized_standing_slope_batch(3,:),-zz,'color','k','LineWidth',lw);
    hold on;
    p2 = plot(normalized_standing_slope_batch(1,:),-zz,'color',blue,'LineWidth',lw);
    p3 = plot(normalized_standing_slope_batch(2,:),-zz,'color',green,'LineWidth',lw);
    p4 = plot(normalized_standing_slope_batch(4,:),-zz,'color',yellow,'LineWidth',lw);
    p5 = plot(normalized_standing_slope_batch(5,:),-zz,'color',orange,'LineWidth',lw);
    hold off;
    ylim([100 845]); 
    xlim([-0.02 0.1])
    axis ij;
...
annotation('textbox',[0.7 panelbottom 0.05 0.05],'String','(c)','interpreter','latex','FontSize',fontsize,'LineStyle','None');
    p1 = plot(uo_slope_batch(3,:),-zz,'color','k','LineWidth',lw);
    hold on;
    p2 = plot(uo_slope_batch(1,:),-zz,'color',blue,'LineWidth',lw);
    p3 = plot(uo_slope_batch(2,:),-zz,'color',green,'LineWidth',lw);
    p4 = plot(uo_slope_batch(4,:),-zz,'color',yellow,'LineWidth',lw);
    p5 = plot(uo_slope_batch(5,:),-zz,'color',orange,'LineWidth',lw);
    s1 = plot(ui_slope_batch(3).*ones(1,70),-zz,'--','color','k','LineWidth',lw);
    s2 = plot(ui_slope_batch(1).*ones(1,70),-zz,'--','color',blue,'LineWidth',lw);
    s3 = plot(ui_slope_batch(2).*ones(1,70),-zz,'--','color',green,'LineWidth',lw);
    s4 = plot(ui_slope_batch(4).*ones(1,70),-zz,'--','color',yellow,'LineWidth',lw);
    s5 = plot(ui_slope_batch(5).*ones(1,70),-zz,'--','color',orange,'LineWidth',lw);
    z_ui = zeros(30,1);
    s1 = scatter(ui_slope_batch(3),z_ui(3),sz,'k','o','filled'); 
    s1 = scatter(ui_slope_batch(1),z_ui(1),sz,blue,'o','filled'); 
    s1 = scatter(ui_slope_batch(2),z_ui(2),sz,green,'o','filled'); 
    s1 = scatter(ui_slope_batch(4),z_ui(4),sz,yellow,'o','filled'); 
    s1 = scatter(ui_slope_batch(5),z_ui(5),sz,orange,'o','filled'); 
    hold off;
    ylim([0 890]); 
    xlim([-0.38 -0.15])
    axis ij;
...
leg1 = legend([p2 p3 p1 p4 p5],...
    '$A_\mathrm{tide}=0$ m/s',...
    '$A_\mathrm{tide}=0.025$ m/s',...
    '$A_\mathrm{tide}=0.05$ m/s (Ref.)',...
    '$A_\mathrm{tide}=0.075$ m/s',...
    '$A_\mathrm{tide}=0.1$ m/s', 'FontSize', fontsize-1,'interpreter','latex');
set(leg1,'position',[0.1598 0.5717 0.1760 0.1925])
legend boxoff                
        

print('-dpng','-r150','Feddy_ifs_Atide_normalized.png');
