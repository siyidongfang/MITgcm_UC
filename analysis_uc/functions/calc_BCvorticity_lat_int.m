...
%     figdir = ['/Users/csi/MITgcm_UC/figures_uc/BCvorticity_cdw_sw/' exp_group '/'];
    ...
    for ii=1:Nx
        for jj=1:Ny
            if(XX(ii,jj)>=50*m1km ...
                    || ((XX(ii,jj)<=-50*m1km) && (YY(ii,jj)<=210*m1km)) ...
                    || YY(ii,jj)<=50*m1km ...
                    || XX(ii,jj)<=-120*m1km ...
                    || Amask(ii,jj)<=-1500) ...
                    || (YY(ii,jj)>=150*m1km && Amask(ii,jj)>=-580) ...
                    || (YY(ii,jj)>=220*m1km && XX(ii,jj)>=30*m1km) ...
                Amask(ii,jj)=NaN;
            end
        end
    end
    ...
    figure(1)
    clf;set(gcf,'color','w');
    pcolor(XX/1000,YY/1000,Amask);shading flat;
    ...
    BPT_Aint = cumsum(sum(zeta_BPT.*Amask*dx*dy,'omitnan'));
    IPT_Aint = cumsum(sum(zeta_IPT.*Amask*dx*dy,'omitnan'));
    Advec_Aint = cumsum(sum(zeta_Advec.*Amask*dx*dy,'omitnan'));
    Diss_Aint = cumsum(sum(zeta_Diss.*Amask*dx*dy,'omitnan'));
    residual_Aint = cumsum(sum(zeta_residual.*Amask*dx*dy,'omitnan'));

    figure(12)
    clf;set(gcf,'color','w');
    plot(yy/1000,BPT_Aint,'-.','LineWidth',2)
    hold on;
    plot(yy/1000,IPT_Aint,'-.','LineWidth',2)
    plot(yy/1000,BPT_Aint+IPT_Aint,'LineWidth',2)
    plot(yy/1000,Advec_Aint,'LineWidth',2)
    plot(yy/1000,Diss_Aint,'LineWidth',2)
    plot(yy/1000,residual_Aint,'--','LineWidth',2,'Color',gray)
